...
class IntelligentScrapingAgent:
    ...
    async def process_chat_message(self, session_id: str, message: str, urls: Optional[List[str]] = None) -> str:
        ...
        if any(keyword in message_lower for keyword in ['scrape', 'extract', 'get data', 'crawl']):
            if urls:
                return await self._handle_scraping_request(session_id, message, urls)
            else:
                return "I'd be happy to help you scrape data! Please provide the URLs you'd like me to process."
        
        elif any(keyword in message_lower for keyword in ['analyze', 'check', 'examine']):
            if urls and len(urls) == 1:
                return await self._handle_analysis_request(urls[0], message)
            else:
                return "To analyze a website, please provide a single URL you'd like me to examine."
        
        elif any(keyword in message_lower for keyword in ['job', 'status', 'progress']):
        ...
        elif any(keyword in message_lower for keyword in ['export', 'download', 'save']):
